...
class NewPoliceServices
{
    ...
    public function __construct(Invoices $invoices)
    {
        ...
        $this->client = Client::where('id', $this->invoices->client_id)->firs();
    }
    ...
    public function generatePolice()
    {
        ...
        if ($newPolice->status == 00) {
            $this->invoices->policy_number = $newPolice->insurancePolicyNumber;
    ...
    private function postpolice($json)
    {
        try {
            ...
            $error = $e->getMessage();
            Log::error("Generar Poliza  ", ['error' => $error]);

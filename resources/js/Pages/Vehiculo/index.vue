    ...
    <div class="bg-gray-200 pb-6" :class="{ 'animate-pulse': Loading, 'opacity-50': Loading }">
        ...
        <section class="p-3 relative rounded-xl bg-white mx-3 z-50 mt-4">
            ...
            <div
                ...
                <div class="mt-2 py-2">
                    <div class="text-left text-2xl font-bold sm:text-2xl md:text-3xl xl:text-3xl">
                        Datos del Vehículo
                    </div>
                </div>
                ...
                <form @submit.prevent="submit" class="flex flex-col">
                    ...
                    <label class="pt-1 justify-start font-bold">Tipo de Vehículo
                        <span class="text-red-400 inl">*</span></label>

                    <select class="rounded-lg w-full border-gray-300" v-model="form.tipo">
                        <option :value="car.tipoName" disabled selected hidden v-if="car.tipoName != ''">
                            {{ car.tipoName }}
                        </option>
                    ...
                    <div class="relative">
                        <input class="rounded-lg w-full border-gray-300" :class="{ 'invalid': v$.form.marca.$error }"
                            style="text-transform: uppercase" type="text" placeholder="MARCA" v-model="form.marca"
                            @input="filterMarcas" @focus="handleFocus" @blur="handleBlur" />
                        <span v-if="v$.form.marca.$error" class="text-red-500">{{ v$.form.marca.$errors[0].$message
                            }}</span>
                        <div v-if="filteredMarcas.length > 0 && showDropdown"
                            ...
                            <ul class="py-2">
                                <li v-for="marca in filteredMarcas" :key="marca.ID" :value="marca.ID"
                    ...
                    <div class="relative">
                        ...
                        <model-list-select class="selectSearch" :class="{ 'invalid': v$.form.modelo.$error }"
                            v-model="form.modelo" :value="car.modeloName" :list="!models ? [{}] : models"
                            :key="car.modeloName" option-value="ID" option-text="descripcion" placeholder="MODELO">
                        </model-list-select>
                    </div>
                    <span v-if="v$.form.modelo.$error" class="text-red-500">{{ v$.form.modelo.$errors[0].$message
                        }}</span>

                    <label class="pt-1 font-bold">Año <span class="text-red-400 inl">*</span></label>
                    <select class="rounded-lg w-full border-gray-300" v-model="form.year" required>
                        <option :value="car.year" disabled selected hidden v-if="car.year != ''">
                            {{ car.year }}
                        </option>
                        <option value="" disabled selected hidden v-else>
                            AÑO
                    ...
                    <label class="pt-1 font-bold">No. de Placa
                        <span class="text-red-400 inl">*</span></label>
                    <input class="rounded-lg w-full border-gray-300" style="text-transform: uppercase"
                        :class="{ 'invalid': v$.form.placa.$error }" type="text" maxlength="10" placeholder="PLACA"
                        v-model="form.placa" />
                    <span v-if="v$.form.placa.$error" class="text-red-500">{{ v$.form.placa.$errors[0].$message
                        }}</span>


                    <label class="pt-1 font-bold">No. de Chasis <span class="text-red-400 inl">*</span>
                    </label>
                    <input class="rounded-lg w-full border-gray-300" :class="{ 'invalid': v$.form.chasis.$error }"
                        style="text-transform: uppercase" type="text" maxlength="17" placeholder="CHASIS"
                        v-model="form.chasis" @input="cleanSpaces" />
                    <span v-if="v$.form.chasis.$error" class="text-red-500">{{ v$.form.chasis.$errors[0].$message
                        }}</span>

                    <div class="w-full mt-5 mx-5 my-4 justify-self-center self-center text-center">
                        <button :disabled="isTipoCarroEmpty ||
                            isMarcaEmpty ||
                            isModeloEmpty
                            "
...
export default {
    ...
    computed: {
        ...
        isValidPlate(){
            ...
            this.selectedType?  this.selectedType : this.correctVehicleType = this.vehicletype.map(x => x.Name)
        ...
        selectedVehicleType() {
            return this.tipos.find(t => t.id === this.form?.tipo);
        }
    },
    methods: {
        async submit() {
            const isInvalid = this.v$.form.$invalid
            const isValidVehicleType = this.vehicletype.map(x => x.Id).includes(this.selectedVehicleType.id)
            if(!isValidVehicleType){
                swal("Error", `El tipo de vehiculo seleccionado no concuerda con el modelo seleccionado. \n El tipo de vehiculo permitido es: ${this.correctVehicleType.join(", ")} `, "error");
                if(this.correctVehicleType.length === 1){
                    this.newtype = this.vehicletype.find(x => x.Name === this.correctVehicleType.join(", "))
                    this.form.tipo = this.newtype.Id
                }
            }
            if (isInvalid) {
